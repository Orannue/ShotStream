...
class BaseModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        ...
        print(f"Check scheduler's timestep shift is {self.scheduler.shift}")
...
class SelfForcingModel(BaseModel):
    ...
    def _run_generator(
        ...
        dist.broadcast(num_generated_blocks, src=0)  # [NOTE] Must generate same blocks to ensure the DDP is right
        ...
        if self.args.independent_first_frame and initial_latent is None:
            ...
            min_num_frames += 1
        # [NOTE] Sync num_generated_frames across all processes
...
class SelfForcingFrameConcatModel(BaseModel):
    def __init__(self, args, device):
        super().__init__(args, device)
        self.denoising_loss_func = get_denoising_loss(args.denoising_loss_type)()
    ...
    def _run_generator(
        ...
        initial_latent: torch.tensor = None,
        slice_last_frames: int = 21,
        context_latent: torch.tensor = None,  # condition latents, which contains previous shots' information 
        ...
        pred_image_or_video, denoised_timestep_from, denoised_timestep_to = self._consistency_backward_simulation(
            noise=torch.randn(noise_shape,
                              device=self.device, dtype=self.dtype),
            slice_last_frames=slice_last_frames,
            context_latent=context_latent,  # add for condition latents
    ...
    def _consistency_backward_simulation(
        ...
        noise: torch.Tensor,
        slice_last_frames: int = 21,
        context_latent: torch.Tensor=None,  # add for condition latents, which contains previous shots information
        shot_flags_for_rope: torch.Tensor=None,
        **conditional_dict: dict,
    ) -> torch.Tensor:
        """
        ...
        return self.inference_pipeline.inference_with_trajectory(
            noise=noise, **conditional_dict, slice_last_frames=slice_last_frames, context_latent=context_latent, shot_flags_for_rope=shot_flags_for_rope
        )
    ...
    def _initialize_inference_pipeline(self):
        ...
        from pipeline import SelfForcingFrameconcatTrainingPipeline

        self.inference_pipeline = SelfForcingFrameconcatTrainingPipeline(
            denoising_step_list=self.denoising_step_list,
            scheduler=self.scheduler,
            generator=self.generator,
            num_frame_per_block=self.num_frame_per_block,
...
class FrameConcatModel(nn.Module):
    def __init__(self, args, device):
        super().__init__()
        self.model_name = getattr(args, "model_name", "Wan2.1-T2V-1.3B")
        self.model_root = getattr(args, "model_root", None)
        ...
        if self.num_frame_per_block > 1:
            ...
            print(f"[Init] the num_frame_per_block of the model is {self.generator.model.num_frame_per_block}")
    ...
    def _get_timestep(
            ...
            - If not uniform_timestep, it will use a different timestep for each block.
            """
            timestep = torch.randint(  # only uniform sample for t2v training
                min_timestep,
                max_timestep,
                [batch_size, 1],
                device=device,
...
class FrameConcatCausalModel(nn.Module):
    def __init__(self, args, device):
        super().__init__()
        self.model_name = getattr(args, "model_name", "Wan2.1-T2V-1.3B")
        self.model_root = getattr(args, "model_root", None)
        ...
        print(f"[Init] the timesteps of the model is {self.denoising_step_list}")
