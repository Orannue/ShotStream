...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_root="wan_models/Wan2.1-T2V-1.3B") -> None:
        super().__init__()
        ...
        self.text_encoder.load_state_dict(
            torch.load(self.model_root / "models_t5_umt5-xxl-enc-bf16.pth",
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=str(self.model_root / "google" / "umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, model_root="wan_models/Wan2.1-T2V-1.3B"):
        super().__init__()
        self.model_root = Path(model_root)
        mean = [
        ...
        self.model = _video_vae(
            pretrained_path=str(self.model_root / "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
        self.model.z_dim = 16
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        model_path = Path(model_root) if model_root is not None else Path("wan_models") / model_name
    ...
    def forward(
        ...
        ###
        frameconcat_infer: Optional[bool] = False,
        causal_use_condition_mask: Optional[bool] = False,
        ...
        logits = None
        # X0 prediction
        if kv_cache is not None or kv_cache_context is not None:  # add for context tokens
            ...
            if clean_x is not None:
                ...
                if classify_mode:
                    ...
                    if causal_use_condition_mask is True:  # add for conditional input
                        flow_pred = self.model(
